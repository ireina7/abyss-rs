...
fn eval_atom(op: Object, a: Object, b: Object, env: &mut Env, backtrace: &mut Backtrace) -> Result<Object> {
    ...
    match op {
        Var(s) => {
            let mut xs = vec![];
            xs.push(evaluate(a, env, backtrace)?);
            xs.push(evaluate(b, env, backtrace)?);
            match &s[..] {
                "+" | "-" | "*" | "/" => eval_arith(&s, &xs),
                "<"  => eval_lt(&xs),
                ">"  => eval_gt(&xs),
...
fn eval_cases(expr: Object, cases: &[Object], env: &mut Env, backtrace: &mut Backtrace) -> Result<Object> {
    ...
    Err(EvalError::new(format!("Case expression error"), backtrace.clone()))
}
...
fn apply(f: Object, x: Object, backtrace: &mut Backtrace) -> Result<Object> {
    ...
    match &f {
        ...
        _ => Err(EvalError::new(format!("Function application error: {:?}", f), backtrace.clone()))
    }
}
...
pub fn evaluate(expr: Object, env: &mut Env, backtrace: &mut Backtrace) -> Result<Object> {
    ...
    let ans = match expr {
        ...
        Thunk(_, _, _) => eval_thunk(expr, backtrace),
        List(xs)       => {
            ...
            match op.unwrap() {
                ...
                f if it.clone().peekable().peek() == None => {
                    let f = evaluate(f, env, backtrace)?;
                    apply(f, Nil, backtrace)
                },
                f /* normal function application */ => {
                    let mut f = evaluate(f, env, backtrace)?;
                    for x in it {
                        //let xv = evaluate(x, env)?;
                        let x = evaluate(x, env, backtrace)?;
        ...
        _ => Err(EvalError::new(format!("Unknow expression: {:?}", expr), backtrace.clone()))
    };
    ans
}
...
mod tests {
    ...
    fn simple_recursive() {
        ...
        if let Ok(ast) = src.parse::<Object>() {
            let res = evaluate(ast, &mut env, &mut log);
